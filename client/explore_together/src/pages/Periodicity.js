...
const Periodicity = observer(() => {
	...
	return (
		<Layout>
			{
				AdminInstance._isEditing !== '' && <UpdateModal document={'Periodicities'} records={periodicitiesById} load={load} updateFunc={updatePeriodicities}/>
			}
			{
				AdminInstance._isDeleting!=='' && <DeleteModal document={'Periodicities'} load={load} deleteFunc={deletePeriodicities}/>
			}
			{
				AdminInstance._isCreating && <CreateModal document={'Periodicities'} load={load} createFunc={createPeriodicities}/>
			}
			<Sidebar />
			<button onClick={()=>{

...
const Level = observer(() => {
	...
	return (
		<Layout>
			{
				AdminInstance._isEditing !== '' && <UpdateMiddle document={'Levels'} records={levelsById} load={load} dependencies={sections} dependName={'sectionId'} updateFunc={updateLevels}/>
			}
			{
				AdminInstance._isDeleting!=='' && <DeleteModal document={'Levels'} load={load} deleteFunc={deleteLevels}/>
			}
			{
				AdminInstance._isCreating && <CreateMiddle document={'Levels'} dependencies={sections} dependName={'sectionId'} load={load} createFunc={createLevels}/>
			}
			<Sidebar />
			<button onClick={()=>{

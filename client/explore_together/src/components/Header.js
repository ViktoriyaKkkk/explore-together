...
const Header = observer(() => {
	...
	return (<header>
			<nav
				className='fixed z-10 top-0 max-h-14 w-full px-4 lg:px-6 pt-1 pb-2.5 opacity-90 bg-black flex flex-wrap justify-between items-center w-screen'>
				<Link to='/' className='flex font-bold'>
					...
					<span className='whitespace-nowrap text-white'>ExploreTogether</span>
				</Link>

				{
					...
					<div className='hidden justify-between w-full sm:flex sm:w-auto' id='mobile-menu-2'>
						<ul className='flex flex-col mt-4 font-medium lg:flex-row lg:space-x-3 lg:mt-0'>
							<Link to='/'
										className={clsx('pr-4 pl-3 text-white',
								...
								<>
									...
									<Link to={`/dialogs/${id}`}
												className={clsx('pr-4 pl-3 text-white',
													...
													'border-b-2 border-b-dark-green')}>Диалоги</Link>
								</>
							}
				...
				<div className='flex'>
					...
					{userStore._isAuth && userStore._user.role === '641e18b855a5d5389d78aba7' &&
						<IconContext.Provider value={{ size: '1.8em' }}>
							<button className='px-2.5 py-1.5 text-white' onClick={() => {

...
const StartForm = observer(({ isLoginStore }) => {
            ...
            bg-white rounded-lg shadow-md lg:shadow-lg'>
				...
				<form className='mt-10' method='POST'>
                    ...
                    focus:outline-none hover:bg-gray-700 hover:shadow-none'>
						{ isLoginStore.isLogin ? "Войти" : 'Зарегистрироваться'}
					</button>
					...
					<div className='sm:flex sm:flex-wrap mt-8 sm:mb-4 text-sm text-center'>
						...
						<p className='flex-1 text-gray-500 text-md mx-4 my-1 sm:my-auto'>
							{
								isLoginStore.isLogin ? 'Ещё нет аккаунта?' : 'Уже есть аккаунт?'
							}

						</p>

						<Link href='/' onClick={()=>isLoginStore.setIsLogin(!isLoginStore.isLogin)} className='flex-2 text-black underline'>
							{
								isLoginStore.isLogin ? 'Зарегистрироваться' : 'Войти'
							}
						</Link>
					</div>
				</form>
